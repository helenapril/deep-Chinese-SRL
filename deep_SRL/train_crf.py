...
from model_crf import SRLModel
from eval import eval
from inference_crf import recover
    ...
    with tf.variable_scope('SRLModel'):
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            for step in range(num_batches_train):
                ori_inputs, scores, golds, lens, batch_loss, matrix, _, g_step = sess.run([
                    train_cnn.ori_inputs,
                    train_cnn.scores,
                    train_cnn.labels,
                    train_cnn.seq_length,
                    train_cnn.loss,
                    train_cnn.transition_params,
                    apply_gradient_op,
                    global_step])

                loss += batch_loss
                avg_cur_loss = loss / (step + 1)
                inputs, preds, golds = recover(ori_inputs, scores, golds, lens, matrix)
        ...
        writer = codecs.open('loss_3', 'w', encoding='utf-8')
