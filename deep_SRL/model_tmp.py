...
from highway_ln_lstm import LNLSTMCell, LNHighwayLSTMCell
...
'''
   compared to model.py, add layer_norm choice
'''
...
class SRLModel():
    ...
    def __init__(self, inputs, preds, contexts, marks, labels, ori_inputs,
                 ...
                 LN_mode, dropout_mode, is_training):
        ...
        with tf.variable_scope('get_cell', initializer=tf.orthogonal_initializer):
            ...
            def lstm_cell():
                if self.LN_mode:
                    return LNLSTMCell(self.hidden_size, state_is_tuple=True, forget_bias=1.0)
                else:
            ...
            def highway_cell():
                if not self.LN_mode:
                    return HighwayLSTMCell(self.hidden_size, state_is_tuple=True, forget_bias=1.0)
                else:
                    return LNHighwayLSTMCell(self.hidden_size, state_is_tuple=True, forget_bias=1.0)
